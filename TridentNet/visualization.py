...
def my_dataset_function(txt_dir="./half_data_test.txt"):
    ...
    for i in range(data_lens):
        ...
        bbox_json_file = "/ssd/JH/" + data_files[i][0].split("media")[1] + "/Learning_Data/bbox/" + data_files[i][1][:-4] + ".json"
        poly_json_file = "/ssd/JH/" + data_files[i][0].split("media")[1] + "/Learning_Data/poly/" + data_files[i][1][:-4] + ".json"
...
test_dict = my_dataset_function(txt_dir="./half_data_test.txt")
#print(test_dict)
...
SAVE_PATH = '/ssd/JH/result_middle'
def main():
    ...
    for idx in range(len(test_dict)):#tqdm(paths):
        ...
        cv2.imwrite('/ssd/JH/result_orig/' + test_dict[idx]["file_name"].split("/")[-1] , im_orig) 
        cfg = get_cfg()
        add_tridentnet_config(cfg)
        cfg.merge_from_file("/ssd/JH/detectron2_t/projects/TridentNet/configs/tridentnet_fast_R_50_C4_1x.yaml")
        cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.5
        cfg.MODEL.WEIGHTS = "/ssd/JH/detectron2_t/projects/TridentNet/output/model_final.pth"
        ...
        plt.axis('off')
        plt.savefig(os.path.join(save_dir, _name), bbox_inches='tight', pad_inches=0)
